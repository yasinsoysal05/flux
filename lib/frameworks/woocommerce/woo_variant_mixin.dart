...
import '../../models/index.dart'
    show AppModel, Attribute, Product, ProductAttribute, ProductVariation;
...
mixin WooVariantMixin on ProductVariantMixin {
  Future<void> getProductVariations({
    ...
    await Services().getProductVariations(product, lang: lang).then((value) {
    ...
    if (variations.isEmpty) {
      ...
      await Services().getProduct(product.id, lang: lang).then((onValue) {
        if (onValue != null) {
          productInfo = onValue;
        }
      });
      ...
      if (productInfo.defaultAttributes.isEmpty) {
        ...
        for (var variant in variations) {
          ...
          if (mapAttribute.isEmpty) {
            ProductVariation firstItem = variations[0];
        ...
        for (Attribute attribute in productInfo.defaultAttributes) {
      ...
      for (ProductVariation item in variations) {
  ...
  bool couldBePurchased(
    ...
    final bool isValidProductVariant = productVariation != null
  ...
  List<Widget> getProductAttributeWidget(
    ...
    final bool checkProductAttribute = product.attributes?.isNotEmpty ?? false;
    if (checkProductAttribute) {
      for (ProductAttribute attr in product.attributes) {
        if (attr.name?.isNotEmpty ?? false) {
          ...
          final String attrType =
              ProductVariantLayout[attr.cleanSlug ?? attr.name] ??
  ...
  List<String> _getValidAttributeOptions(ProductAttribute attr,
    ...
    return List<String>.from(
      attr.options
          .map((e) {
            Map<String, String> copy = Map.from(mapAttribute);
