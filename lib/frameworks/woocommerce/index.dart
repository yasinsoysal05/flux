...
class WooWidget
  ...
  Future<void> onLoadedAppConfig(String lang, Function callback) async {
    final countries = await Services().getCountries();
    ...
    if (kAdvanceConfig['isCaching']) {
      final configCache = await Services().getHomeCache(lang);
      if (configCache != null) {
...
//    final cartModel = Provider.of<CartModel>(context, listen: false);
  ...
  Future<List<CountryState>> loadStates(Country country) async {
    ...
    if (items != null && items.isNotEmpty) {
      ...
      try {
        final items = await Services().getStatesByCountryId(country.id);
        if (items != null && items.isNotEmpty) {
          for (var item in items) {
  ...
  Future<void> syncCartFromWebsite(String token, BuildContext context) async {
    try {
      List items = await Services().getCartInfo(token);
      if (items != null && items.isNotEmpty) {
        final cartModel = Provider.of<CartModel>(context, listen: false);
        cartModel.clearCart();
  ...
  Future<void> syncCartToWebsite(CartModel cartModel) {
    if (kAdvanceConfig["EnableSyncCartToWebsite"] == true) {
      return Services().syncCartToWebsite(cartModel, cartModel.user);
    }
    return null;
  }
