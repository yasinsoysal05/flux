...
class HomeScreenState extends BaseScreen<HomeScreen>
  ...
  static ShowNotificationOffer(BuildContext context) async {
    if (OneSignalWapper.hasNotificationData &&
      ...
      for (int i = 0; i < OneSignalWapper.productIdArray.length; i++) {
        Product newProduct =
            await _service.getProduct(OneSignalWapper.productIdArray[i]);
        loadedProducts.add(newProduct);
      }
      hideLoading();
      String loadedCategoryID = OneSignalWapper.categoryID;
